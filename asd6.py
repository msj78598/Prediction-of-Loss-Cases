# ===================== نظام تحليل الفقد الذكي (قراءة فقط + تصدير الحالات المكتشفة فقط) =====================
...
# ============ كاش تحميل النموذج (مرة واحدة) ============
@st.cache_resource
def load_model_once():
    if not os.path.exists(MODEL_PATH):
        st.error("⚠️ ملف النموذج غير موجود (ASD6.pkl). ارفعه في المسار المحدد.")
        st.stop()
    return joblib.load(MODEL_PATH)

model = load_model_once()  # يحمَّل مرة واحدة فقط لكل جلسة

# ============ كاش قراءة البيانات ============
@st.cache_data
def read_excel_cached(upload):
    df = pd.read_excel(upload)
    # تحويل الميزات لتقليل الذاكرة
    for c in ["V1","V2","V3","A1","A2","A3"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce").astype("float32")
    return df

# ============ إعدادات/محددات قابلة للضبط ============
...
def detect_two_phase_strict(row):
    ...
    # 1) فازة مرشحة كـ "غير مستخدمة": V≈0 أو I≈0 مع V طبيعي
    candidates = []
    for i in range(3):
    ...
    # 2) الفازة غير المستخدمة فعلاً ضعيفة مطلقًا ونِسبيًا
    ...
    # 4) إلغاء صريح: إذا الثلاث فازات محملة بشكل ملحوظ → ليس ثنائي فاز
...
def analyze_data(df):
    try:
        ...
        mask_tp = df['Two_Phase_Meter'] == True
        df.loc[mask_tp, 'Reason'] = df.loc[mask_tp].apply(
        ...
        # إزالة تكرار العدادات (نأخذ الأعلى خطورة)
        ...
        logical_only = df[df['Case_Type'].str.contains("هندسي")]  # حالات تنطبق عليها المحددات ولم يكتشفها النموذج
        ...
        st.subheader("🚨 حالات الفقد ذات الأولوية العالية (سبب يحتوي ⚠️)")
        st.dataframe(high_priority)

        st.subheader("🧠 حالات تنطبق عليها المحددات ولم يكتشفها النموذج")
        st.dataframe(logical_only)

        st.subheader("🔴 الحالات الأعلى تأثيرًا وخطورة (Severity ≥ 0.7)")
        st.dataframe(high_impact)

        st.subheader("ℹ️ عدادات ثنائية الفاز (مستثناة)")
        st.dataframe(two_phase_only)

        # ========== تصدير Excel للحالات المكتشفة/المهمّة فقط ==========
        # (لا نصدر “السليم” لتخفيف الضغط)
        output = BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            # صدّر فقط الجداول المهمة؛ احذف إذا كبيرة جدًا لتخفيف الاستهلاك
        ...
        st.download_button(
            "📥 تحميل تقرير الحالات المكتشفة فقط (Excel)",
            data=output,
            file_name="loss_detected_report.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

        # تنظيف ذاكرة مؤقتًا
        del detected_loss, high_priority, logical_only, two_phase_only, high_impact
        gc.collect()
...
# ============ واجهة التطبيق ============
st.sidebar.title("📂 القوالب")
if os.path.exists(DATA_TEMPLATE):
    with open(DATA_TEMPLATE, "rb") as f:
    ...
    st.sidebar.warning("⚠️ قالب البيانات غير متوفر في المسار.")

st.title("⚡ نظام ذكي لتحليل واكتشاف حالات الفقد")
st.markdown("يحلّل ملف الأحمال ويصدّر **Excel للحالات المكتشفة فقط** لتقليل الضغط.")
